...
### top arm hits and heatmaps from linear regressions (both hot and cold hits)
get_top_linearreg(reg_out=all_arm_linreg_results, AS_df = armAS_mean, remove = remove, cnvlevel = "Arm", 
                  ...
                  savepath = "IS_CNV_Regression/TCGA/Linear_Regression_TopHits/ArmCNV_linreg_tophits", savetable = T) 

full_arm_df <- data.frame()
for(tt in names(all_arm_linreg_results)) {
  d <- all_arm_linreg_results[[tt]]
  full_arm_df <- rbind(full_arm_df, d)
}
full_arm_df$ImmuneHit <- ifelse(full_arm_df$plot_hit < 0, "Immune Cold", ifelse(full_arm_df$plot_hit > 0, "Immune Hot", ""))
write.csv(full_arm_df, "IS_CNV_Regression/TCGA/Linear_Regression_TopHits/ArmCNV_linreg_results.csv", row.names = F)
